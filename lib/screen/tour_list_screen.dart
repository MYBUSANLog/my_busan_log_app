import 'package:busan_trip/app_http/item_free_http.dart';
...
class _TourListScreenState extends State<TourListScreen> {
  late Future<List<Item_free>> _freeitem; // 데이터를 불러오기 위한 Future 객체
  ...
  void initState() {
    ...
    _freeitem = ItemFreeHttp.fetchFree( sortBy, 0, itemsPerPage); // 첫 페이지 데이터를 불러옴
    _cnum = ItemFreeHttp.fetchNum(); //
  }
  ...
  Future<void> _loadMoreItems() async {
    ...
    List<Item_free> newItems = await ItemFreeHttp.fetchFree(sortBy, currentPage * itemsPerPage, itemsPerPage);
    ...
    setState(() {
      favoriteCards.addAll(newItems.asMap().entries.map((entry) {
        ...
        Item_free item_free = entry.value;
        return FavoriteCard(item_free: item_free, index: index, );  // index와 item을 함께 전달
      }).toList());
      currentPage++; // 페이지 증가
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return FavoriteCard(item_free: favoriteCard.item_free, index: index + 1, ); // 인덱스를 1씩 증가시켜 전달
              },
            ),
          ),
          SizedBox(height: 10),
...
class FavoriteCard extends StatefulWidget {
  final Item_free item_free;
  final int index;

  FavoriteCard({required this.item_free, required this.index});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        // Navigator.push(
        //   context,
        //   MaterialPageRoute(builder: (context) => ItemDetailScreen(item_free: item_free,)),
        // );
      },
      child: Container(
        ...
        child: Row(
          children: [
            ...
            ClipRRect(
              ...
              child: Image.network(
                widget.item_free.i_image, // 아이템 이미지
                width: 75,
                height: 75,
                fit: BoxFit.cover,
              ),
                ...
                children: [
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          widget.item_free.i_name, // 아이템 이름
                  ...
                  Row(
                    children: [
                      Text(
                        '${_formatAddress(widget.item_free.i_address)} · ',
                        style: TextStyle(fontSize: 12, color: Colors.grey),
                      ),
                      SizedBox(width: 5),
                      // Icon(Icons.star_rounded, size: 16, color: Colors.amber), // 별 아이콘 추
                    ...
                    child: Align(
                      ...
                      child: Container(width: 100,
                        child: Text(
                          ...
                          '${widget.item_free.i_free}~',  // 아이템 가격
